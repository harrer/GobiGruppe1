...
combined$per_partly <- combined$partly / (combined$partly + combined$partlynot)
combined$per_complete <- combined$complete / (combined$complete + combined$completenot)

require(ggplot2)

png("all.png",width=1280,height=800)
ggplot(combined,aes(x=length,y=all)) + geom_point() + scale_y_log10() + scale_x_log10() + xlab("Length of spliceevent")
dev.off()

png("partly.png",width=1280,height=800)
ggplot(combined,aes(x=length,y=per_partly)) + geom_point() + scale_x_log10() + xlab("Length of spliceevent")
dev.off()

png("complete.png",width=1280,height=800)
ggplot(combined,aes(x=length,y=per_complete)) + geom_point() + scale_x_log10() + xlab("Length of spliceevent")
dev.off()
